...
function shouldRunMigrations() {
  const value = String(process.env.MIGRATE_ON_START ?? "").toLowerCase();
...
export function ensurePrismaEngine() {
...
async function ensureOvertimeEligibleColumn() {
  try {
    ...
    if (!hasColumn) {
      ...
      appendMigrationLog(`Patched schema: added Employee.overtimeEligible`);
    }
  } catch (err) {
...
async function ensureSalaryColumn() {
  try {
    ...
    if (!hasColumn) {
      ...
      appendMigrationLog(`Patched schema: added Employee.salary`);
    }
  } catch (err) {
  ...
  for (const id of entries) {
    ...
    try {
      ...
      try {
        ...
        // Rollback may fail if transaction was not opened.
      }
...
export async function bootstrapDatabaseOnStart() {
  ensurePrismaEngine();
  ...
  // Permanent compatibility guard for old SQLite files that missed a migration.
