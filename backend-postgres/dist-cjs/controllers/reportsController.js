...
exports.updateSalaryAdjustmentHandler = updateSalaryAdjustmentHandler;
exports.updateAttendanceRemarkHandler = updateAttendanceRemarkHandler;
...
function formatFlooredOtHours(minutes) {
...
function applyAutoFitWidths(worksheet, columns, rows, options) {
...
async function updateSalaryAdjustmentHandler(req, res, next) {
    try {
        const { employeeId } = req.params;
        const { month, fine, advance, others, arrear, salaryRemark } = req.body;
        if (!employeeId || !month) {
...
async function updateAttendanceRemarkHandler(req, res, next) {
    try {
        const { employeeId } = req.params;
        const { month, remark } = req.body;
        if (!employeeId || !month) {
            return res.status(400).json({ message: "employeeId and month are required." });
        }
        const row = await (0, attendanceService_js_1.upsertAttendanceSummaryRemark)({ employeeId, month, remark: remark ?? null });
        return res.json(row);
    }
    catch (error) {
        return next(error);
...
async function exportAttendanceSummary(req, res, next) {
    try {
        ...
        worksheet.getCell("A6").value = `Month: ${month} | Department: ${department ?? "all"} | Generated: ${(0, date_js_1.dayjs)().format("DD MMM YYYY HH:mm A")}`;
        ...
        applyAutoFitWidths(worksheet, columns, tableRows, { min: 8, max: 34, padding: 2 });
...
async function exportSalarySheet(req, res, next) {
    try {
        ...
        applyAutoFitWidths(worksheet, columns, tableRows, { min: 8, max: 40, padding: 2 });
        for (let c = 1; c <= columns.length; c += 1) {
            const headerCell = worksheet.getCell(8, c);
            headerCell.font = { bold: true, color: { argb: "FFFFFFFF" } };
            headerCell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FF1F4E78" } };
